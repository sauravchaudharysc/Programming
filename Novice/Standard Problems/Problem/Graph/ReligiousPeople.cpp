...
Further there are (x-1) roads required in each component where x is no of vertices in that 
component
...
But there are some vertices that do not form any edges Some vertices which stand alone and 
each of these vertices must have a temple of its 
own Since they do not form any edge , there count is not included in the adjacency list No
of such nodes is (V - n) ...where V is total no of nodes and n is no of nodes that form an
edge and are part of previously counted components Since each of these nodes must have a 
...
public:
    ...
    ll dfsReligiousPeople(ll src, ll a, ll b)
    {
        ...
        // a(cost of temple)*components where,components :- disconnected cities So temple==disconnected cities.
        //b(cost of road)*(n - components)
        return totalCost;
    }
};
