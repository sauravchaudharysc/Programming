...
Time to print the 3rd nearest distance ( k=3 )
Output : 98*/
/*Insert only k minimum distance element in maxHeap.It is maxHeap so the maximum is on the top.
So after k insertion for each insertion check is the top greater than current distance if yes
insert the element in the heap. And pop the top.This assures the solution will be in O(N)*/
